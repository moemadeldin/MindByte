...
final class TeacherRegistrationController extends Controller
{
    ...
    public function register(StoreTeacherRequest $request, TeacherRegistrationAction $action): RedirectResponse
    {
        $user = $action->execute(TeacherRegistrationDTO::fromArray($request->validated()));
        Auth::login($user);
